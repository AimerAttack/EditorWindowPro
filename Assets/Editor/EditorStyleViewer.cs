...
namespace Editor
{
    public class EditorStyleViewer : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            selSkin = EditorGUI.Popup(new Rect(position.width - 220, 0, 90, 14), "", selSkin, guiSkins.ToArray(),
                "ToolbarDropDown");
            if (GUI.changed)
            {
            ...
            GUI.EndGroup();
            selSearchParam = EditorGUI.Popup(new Rect(position.width - 118, 2, 16, 16), "", selSearchParam,
                SearchParams, "ToolbarSeachTextFieldPopup");
            ...
            scrollPos = GUI.BeginScrollView(new Rect(0, 18, position.width, position.height - 35), scrollPos,
                new Rect(0, 0, position.width - 16, totalSizeofView + 10));
            for (int i = 0; i < initialCount; i++)
            {
                GUIStyle s = eStyles[i];
                if (CanShow(s.name))
                {
                    ...
                    if (GUI.Button(new Rect(0, 18, 60, 14), "Options", "OL Title"))
                    {
                        ...
                        optionsMenu.AddItem(new GUIContent("Display Sample Text"), stylePrefs[selOption].displayText,
                            ShowSampleText);
                        optionsMenu.AddItem(new GUIContent("Enable or Disable"), stylePrefs[selOption].enabled,
                            ToggleDisabled);
                    ...
                    if (stylePrefs[i].displayText)
                    {
                        ...
                        stylePrefs[i].npActive = GUI.Toggle(new Rect(10, 41, sP.W / 2 + size.x, size.y),
                            stylePrefs[i].npActive, stylePrefs[i].text, s);

                        if (EditorGUIUtility.isProSkin && selSkin == 1)
                        {
                            stylePrefs[i].pActive = GUI.Toggle(
                                new Rect(30 + sP.W / 2 + size.x, 41, sP.W / 2 + size.x, size.y), stylePrefs[i].pActive,
                    ...
                    {
                        GUI.enabled = stylePrefs[i].enabled;
                        stylePrefs[i].npActive =
                            GUI.Toggle(new Rect(10, 41, sP.W, sP.H), stylePrefs[i].npActive, "", s);

                        if (EditorGUIUtility.isProSkin && selSkin == 1)
                        {
                            stylePrefs[i].pActive = GUI.Toggle(new Rect(30 + sP.W, 41, sP.W, sP.H),
                                stylePrefs[i].pActive,
                                "", s.name);
                        }
                    }
